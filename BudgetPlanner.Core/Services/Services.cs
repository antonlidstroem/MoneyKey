...
public class JournalQueryService
{
    ...
    public async Task<(List<JournalEntryDto> Items, int TotalCount, SummaryDto Summary)> QueryAsync(JournalQuery q)
    {
        ...
        // Do NOT use Task.WhenAll here. All repositories inject the same scoped
        // BudgetDbContext. Parallel async operations on one DbContext throw:
        //   "A second operation was started on this context instance before a
        //    previous operation completed."
        // Awaiting each call in sequence is safe and correct.
        ...
        var total = all.Count;
        var summary = ComputeSummary(all); // on ALL items, before pagination
